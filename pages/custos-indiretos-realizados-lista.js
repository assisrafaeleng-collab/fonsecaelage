...
const fmtData = (data) => { if (!data) return '-'; return new Date(data).toLocaleDateString('pt-BR') }
    ...
    <div className="page">
      ...
      <div className="card">
        ...
        {dados.lancamentos.length === 0 ? (
          <div style={{ padding: '20px', textAlign: 'center', color: '#999' }}>Nenhum custo indireto realizado ainda</div>
        ) : (
              ...
              <tbody>
                {dados.lancamentos.map((item, idx) => (
                  <tr key={item.id || idx}>
                    ...
                    <td style={{ textAlign: 'right', fontWeight: '600' }}>{fmtMoeda(item.valor)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
